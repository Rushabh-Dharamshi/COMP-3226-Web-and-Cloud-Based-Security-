...
class PROSDetector:
    ...
    def engineer_features(self, df):
        ...
        df_engineered['commentLikeCount'] = pd.to_numeric(df_engineered['commentLikeCount'], errors='coerce').fillna(0)
    ...
    def find_unattacked_bins(self, df, target, independents):
        ...
        for feature in independents:
            ...
            for bin_val in df[feature].dropna().unique():
                ...
                if len(dist) > 0:  # Only add non-empty distributions
        ...
        print(f"    Collected {len(distributions)} distributions for {target}")
        
        # Cluster Similar Distributions using JSD
        similar_groups = []
        visited = set()
        ...
        if similar_groups:
            ...
            print(f"    ✓ Found cluster of {len(best_group)} similar distributions")
            ...
            for dist in all_dists:
                all_indices.update(dist.index)
            all_indices = sorted(list(all_indices))
            
            # Create aligned arrays
            aligned_arrays = []
            for dist in all_dists:
                aligned = dist.reindex(all_indices, fill_value=0).values
                aligned = aligned / (aligned.sum() + 1e-10)
                aligned_arrays.append(aligned)
            
            # Average
            avg_array = np.mean(aligned_arrays, axis=0)
            avg_array = avg_array / (avg_array.sum() + 1e-10)
            ...
            print(f"    ✗ No similar distribution clusters found")
    ...
    def estimate_clean_distributions(self, df):
        """Estimate clean distributions for all features."""
        ...
        print(f"\n✓ Successfully estimated clean distributions for {len(successful_targets)} features:")
        for target in successful_targets:
            if target in self.clean_distributions:
                print(f"  - {target} ({len(self.clean_distributions[target])} values)")
    ...
    def calculate_anomaly_scores(self, df):
        ...
        def get_clean_prob(feature, val):
            """Get clean probability for a feature value."""
            if feature in self.clean_distributions:
                dist_dict = self.clean_distributions[feature]
                # Handle both string and non-string keys
                key = str(val) if val is not None else 'nan'
                return dist_dict.get(key, 1e-9)
            return 1e-9
        ...
        for col in tuple_cols:
            # Calculate clean probability for each feature
            df_scored[f'p_clean_{col}'] = df_scored[col].apply(
                lambda x: get_clean_prob(col, x) if pd.notna(x) else 1e-9
            ).astype(float)
            
            # Debug: Check if any probabilities are zero
            zero_count = (df_scored[f'p_clean_{col}'] <= 1e-9).sum()
            if zero_count > 0:
                print(f"  Warning: {zero_count} values have near-zero probability for {col}")
            
            # Multiply to get joint probability
            df_scored['P_clean_joint'] = df_scored['P_clean_joint'] * df_scored[f'p_clean_{col}']
        
        # Check for zero probabilities
        zero_joint = (df_scored['P_clean_joint'] <= 0).sum()
        if zero_joint > 0:
            print(f"  Warning: {zero_joint} rows have zero joint clean probability")
        ...
        print("\nCalculating Joint Observed Frequency...")
        
        # Ensure all tuple columns are strings for grouping
        for col in tuple_cols:
            df_scored[f'{col}_str'] = df_scored[col].astype(str).fillna('nan')
        
        tuple_str_cols = [f'{col}_str' for col in tuple_cols]
        
        counts = df_scored.groupby(tuple_str_cols).size().reset_index(name='tuple_count')
        total_rows = len(df_scored)
        counts['P_obs_joint'] = counts['tuple_count'] / total_rows
        
        # Merge back
        df_scored = df_scored.merge(
            counts[tuple_str_cols + ['P_obs_joint']], 
            on=tuple_str_cols, 
            how='left'
        )
        
        # Fill NaN with small probability
        df_scored['P_obs_joint'] = df_scored['P_obs_joint'].fillna(1/total_rows)
        
        # 4. Calculate Odds Score
        print("\nComputing Final Bot Odds...")
        alpha = 0.5  # From PROS paper - α = 0.5 for ranking
        
        # Ensure division happens on floats
        df_scored['P_obs_joint'] = df_scored['P_obs_joint'].astype(float)
        df_scored['P_clean_joint'] = df_scored['P_clean_joint'].astype(float)
        
        # Calculate score with safe division
        with np.errstate(divide='ignore', invalid='ignore'):
            df_scored['pros_anomaly_score'] = (df_scored['P_obs_joint'] / (alpha * df_scored['P_clean_joint'])) - 1
        
        # Handle infinite/NaN values
        df_scored['pros_anomaly_score'] = df_scored['pros_anomaly_score'].replace(
        ...
        print(f"  Min score: {df_scored['pros_anomaly_score'].min():.6f}")
        print(f"  Max score: {df_scored['pros_anomaly_score'].max():.6f}")
        print(f"  Mean score: {df_scored['pros_anomaly_score'].mean():.6f}")
        print(f"  Top 10% threshold: {df_scored['pros_anomaly_score'].quantile(0.9):.6f}")
    ...
    def compare_with_isolation_forest(self, df):
        ...
        try:
            ...
            contamination = 0.1  # Assume top 10% are bots
...
def visualize_results(results):
    ...
    try:
        ...
        plt.show()
        print(f"\nVisualization saved as PNG: {PLOT_OUTPUT_PATH}")
