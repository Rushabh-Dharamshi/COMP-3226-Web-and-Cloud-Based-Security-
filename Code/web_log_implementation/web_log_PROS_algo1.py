...
def find_clean_distribution(subset_data, target_col, bucket_cols, threshold=0.15):
    ...
    """
    clean_subset = subset_data[subset_data['status'].astype(str) == '200'].copy()
    
    # If filtering removed too much, fall back to full subset to avoid crashes
    if len(clean_subset) < 100:
        clean_subset = subset_data.copy()

    # 2. Create Buckets
    clean_subset['bucket'] = clean_subset[bucket_cols].astype(str).agg('-'.join, axis=1)

    bucket_counts = clean_subset['bucket'].value_counts()
    
    # A. Remove "Super-Heavy" Buckets (Likely DDoS/Scrapers)
    # If a single bucket has > 20% of all traffic, it's suspicious. Exclude it from "Normal" learning.
    total_count = len(clean_subset)
    valid_buckets = bucket_counts[bucket_counts < (total_count * 0.20)]
    
    # B. Keep the remaining top 100
    valid_buckets = valid_buckets[valid_buckets > 20].head(100).index
    
    valid_data = clean_subset[clean_subset['bucket'].isin(valid_buckets)]
    # --- OPTIMIZATION END ---
    ...
    n_cols = len(cols)
    if n_cols < 2: return distributions.mean(axis=1)
    ...
    for i in range(n_cols):
        ...
        for j in range(n_cols):
            ...
            try:
                # Optimized: If dists are identical, skip calculation
                if (ref_dist == distributions[cols[j]]).all():
                    dist = 0.0
                else:
...
run_training(
    target='status',
    condition_feature='path_feature',
    independent_features=['browser', 'week', 'family']
)
...
run_training(
    ...
    independent_features=['browser', 'week', 'osFamily']
)
